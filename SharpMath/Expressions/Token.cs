...
namespace SharpMath.Expressions
{
    ...
    public class Token
    {
        ...
        protected static Dictionary<string, Action<Stack<double>>> _functionActions = new Dictionary<string, Action<Stack<double>>>
            {
        ...
        protected static Dictionary<string, Action<Stack<double>>> _operatorActions = new Dictionary<string, Action<Stack<double>>>
        {
        ...
        public static Token<double> ReadNumberToken(string input, ref int index)
        {
            ...
            double numberValue;
            if (!double.TryParse(numberToken, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out numberValue))
        ...
        public static Token<string> ReadStringToken(string input, ref int index)
        {
            TokenType newType;
            int endIndex = index;
            while (endIndex < input.Length && (char.IsLetter(input, endIndex) || CharEx.IsMathematicOperator(input[endIndex]) || CharEx.IsBracket(input[endIndex])))
            {
                endIndex++;
                string currentString = input.Substring(index, endIndex - index);
                if (_functionActions.ContainsKey(currentString) || _operatorActions.ContainsKey(currentString) || currentString.IsBracket()) // Last two conditions to parse e.g. "* sin(5)" correctly
                    break;
            }
            ...
            if (_functionActions.ContainsKey(stringToken))
                ...
                throw new ParserException($"Invalid token: {stringToken} is not a valid function/operator, bracket or number equivalent.");
        ...
        public static IEnumerable<Token> CalculateInfixTokens(string term)
        {
            ...
            {
                char current = term[i];
                if (char.IsLetter(current) || CharEx.IsMathematicOperator(current) || CharEx.IsBracket(current)) // Functions/Operators
                {
                    if ((current == '+' || current == '-') && (i == 0 || (infixTokens.Last().Type == TokenType.Bracket && ((Token<string>)infixTokens.Last()).Value == "("))) // Must be a sign and handled differently.
                    {
                        if (current == '-')
                        {
                            ...
                            term.Remove(term.IndexOf(current), 1); // Remove the '+' as it is redundant and disturbs our calculations
                    }
